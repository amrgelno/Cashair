...
$select_member="select *  from  member  where ID=$MID";
...
<input type='hidden'  name='MID' value='<?php echo $row['ID']; ?>'  /> <!-- SENDER_ID -->

<input type='hidden'  name='UID' value='2'  />  <!-- recevier_ID -->
<textarea type="text"   name="Mssg"  placeholder="insert new topic here"   style="color:#000;" cols="50" rows="10" >
</textarea>  
<br/><br/>
<button type='submit'  name='submit' style= "border: 9px solid #ffffff;
...
<?php
$Client_Name=$row['Company'];  #users Name
$Email=$row['Email'];  #fixed_value  org Email
$recevier_id='2';  # Admin_ID
@$Mssg=$_POST['Mssg'];
$sender_id=$_POST['MID'];  #member_id login

$sql_insert="INSERT INTO `mssg`
    (`Name`, `Email`, `type`, `Mssg`, `sender_id`, `recevier_id`,`img`,`bg_color`, `direction`,`Data`)      
	 VALUES ('$Client_Name','$Email','Client','$Mssg','$sender_id','$recevier_id','client_img.png','#99FF00','rtl',now())";
...
$sender_id=$_POST['MID'];   /*purchaser_id/client - sender*/
