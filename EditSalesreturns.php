...
<title>Sales Returns invoice</title>
<script src="jquery-1.11.0.min.js"></script>
</head>
<style>
...
<div id="loader">
<div class="logo"><img src="img/returnSales.png" class="loading-logo"/></div>
<!--<div id="loading"> </div>
...
class Editselesreturns  extends  condb{        
...
$invs="select *  from  purchase  where  ID=$INV_ID "; 
$invq= $this->connectF()->query($invs);
while($colu = mysqli_fetch_object($invq)){
?>
<BR />

<td><label>INVOICE NO:-</label> <input  type="text" name="INVOID" value="<?php echo $colu->ID
...
<select  name="Client_INV"  required >

<option value="<?php echo  $Client_INV; ?>">

<?php

if($Client_INV == false ){       #$Client_INV ==''
?>  
Select Client
<?php
...
<?php
$select="select * from  client_record ";
...
<option value="<?php  echo $column->Name?>"  ><?php  echo $column->Name
?></option> 
...
$sqq="update  purchase  SET   vendors='$Client_INV'  WHERE  ID ='$nvnoID'  ";
...
onmouseover='calert();' >
<td><label>Item_ID:</label><input type="input" value="<?php echo $row ->ID
?>"  name="Item_ID" style="background-color:#FFF;
width:100px;  border:none;" onmouseover='calert();' /></td>
<td><label>Item:</label><input type="input" value="<?php echo $row ->Item
?>"  name="Item" style="background-color:#FFF;
width:100px;  border:none;" onmouseover='calert();' /></td>
<td><label>cost:</label><input type="input"  value="<?php echo $row ->price
?>"  name="price"  style="background-color:#FFF;
width:100px;  border:none;" onmouseover='calert();' /> </td>
<td><label>Qsupply:</label><input value="<?php echo $row ->Qsupply
?>"  name ="Qsupply"  type="input"  style="background-color:#FFF;width:100px; border:none;" onmouseover='calert();'/></td>
...
<input  type="hidden" name="Client_INV"  value="<?php echo  $Client_INV  ?>"  /></td>
...
values ('$Item','$ItemID','مردودات مبيعات','$Client_INV','$RepName','$cost','$Qsupply','$Qpurchase','$total','$inv',NOW())";
...
$sele="select *  from  pur_order  where InvoiceNo='$nvnoID'  And  invoice_type= 'مردودات مبيعات' ";
...
<td> <Input type="text"  value="<?php  echo $column->Qpurchase
?>"  name="REQRDER"  style="background-color:#FFF; width:100px;  border:none;"  />      </td>
<input  type="hidden" name="REP_INV"  value="<?php echo $column->RepName
?>"   />
<td><input  type="text" name="Client_INV" value="<?php echo $column->vendors
?>"   /></td>
<td> <?php  echo $column->total
?>      </td>
<td>
<input type="hidden"  value="<?php  echo $nvnoID ?>"  name="INVOID" />
<input type="hidden"  value="<?php  echo $column->ID
?>"  name="Itemp_NO" />
<input type="hidden"  value="<?php  echo $column->Item_ID
?>"  name="Itemp_ID" />     
...
$selx="select *  from  purchase  where   ID='$nvnoID'";
...
<input type="hidden"  value="مردودات مبيعات"  name="INVOtype" />

<input type="hidden"  value="<?php  echo $nvnoID ?>"  name="INVOID" />

<input  type="hidden" name="Client_INV" value="<?php echo $column->vendors?>"   />


<input  type="hidden" name="RepName" value="<?php echo $column->RepName?>"   />


<input  type="hidden" name="LAm_paid"  value="<?php echo $column->Am_paid?>" />  <!--LAST Amount --> 


<input  type="text"     name="Amount"  placeholder="<?php echo $column->surplus?>  :مبلغ المتبقي  من الفاتورة"  onkeypress="typeSCc();"    />
...
<input  type="text"    name="Discount" id="Discount"  value="<?php echo $column->Discount?>"   onchange="typeSC();" />

<input  type="text"      id='Netval'   name="Net"   value='<?php echo $column->Net?>'   disabled="disabled" />
...
@$vendors_INV = $_POST['Client_INV'];   //  render client value
