...
$select_ticket="select *  from   ticket  where  UID='$UID'  and  status != 'تم الانتهاء' ORDER BY `ticket`.`ID` DESC  limit $start,$perpage    ";  #login -> select data  / subscribe 	

$sel_count="select count(ID) AS  Number  FROM  ticket where  UID='$UID'   and  status != 'تم الانتهاء'    ";
...
<tr>  <td> نصى المشكلة</td> <td><?php echo $row['issue_Title'] ?> </td> </tr>
