...
<span style='font-weight:bold; background:#06C; color:white; ' > update Seles Invoice  تعديل علي  فاتوره مبيعات    </span> <br />
...
$invs="select *  from  invno where   ID=$INV_ID";
...
<form action="Edit_seles.php"  method="post">  <!--first form -->
...
$sqli="insert into  invoices (Item,Item_ID,invoice_type,QOrder,Qsupply,price,RepName,client,total,InvoiceNo,Time) 
                    values ('$Item','$ItemID','فاتورة مبيعات' ,'$QOrder','$Qsupply','$price','$RepName','$client','$total','$inv',NOW())";
...
$sele="select *  from   invoices where  InvoiceNo=$nvnoID  And   invoice_type='فاتورة مبيعات'";
...
$selx="select *  from invno where ID= '$nvnoID'  ";
...
<input type="hidden"  value="فاتورة مبيعات"  name="INVOtype" />
...
<input  type="text"     name="Amount"  placeholder="<?php echo $column ->surplus?>  :مبلغ المتبقي  من الفاتورة"  onkeyup="typeSCc();"    />
