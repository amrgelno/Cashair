...
if(@$_POST['Sales_Returns']){

$this->connectF()->query($INVQ);
?>
<span style='font-weight:bold; background:#06C; color:white; ' > Creat Salesreturns  Invoice  أنشاء فاتوره مردودات مبيعات</span> <br />
<?php
}
?>
...
<form action="Salesreturns.php" method="post">

<?php

$select="select DISTINCT Item_dep from stock "; 
$sqlq= 
$this->connectF()->query($select);
?>
...
$invq= 
$this->connectF()->query($invs);
while($colu = mysqli_fetch_object($invq)){
?>
<BR />
<input  type="hidden" name="INVOID"   value="<?php echo  $colu->ID ?>" />
<label>REP_Name:</label><input  type="text" name="REP_INV"   value="<?php echo 
$colu->RepName
?>"      placeholder="Rep_Name" style="background-color:#FFF;
...
<td><label>Item:</label><input type="input" value="<?php echo
 $row ->Item
...
width:100px;  border:none;" onmouseover='calert();' /> </td>
<td><label>Qsupply:</label><input value="<?php echo
 $row ->Qsupply
?>"  name ="Qsupply"  type="input"  style="background-color:#FFF;width:100px; border:none;" onmouseover='calert();'/></td>
<td><label>QRETURNS:</label> <input  type="input" name="Qpurchase"  placeholder="الكمية المطلوبة" /></td>
...
<td><input  type="text" name="Client_INV" value="<?php echo $column ->vendors ?>"   /></td>
...
$sum->SUM ?>"  id='total'   name="TOTAL"  disabled="disabled"  /> 
